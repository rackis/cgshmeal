...
def keyboard(request):
    return JsonResponse({
        'type': 'buttons',
        'buttons': ['오늘 식단표', '내일 식단표', '다른 요일 식단표', '3학년 시간표', '학사일정', '오늘의 날씨', '홈페이지 공지사항']
    })
...
def message(request):
    ...
    if meal == '오늘 식단표':
        ...
        return JsonResponse({
            'message': {
                'text': read_txt(meal, today, daystring)
            },
            'keyboard': {
                'type': 'buttons',
                'buttons': ['오늘 식단표', '내일 식단표', '다른 요일 식단표', '3학년 시간표', '학사일정', '오늘의 날씨', '홈페이지 공지사항']
        ...
        return JsonResponse({
            'message': {
                'text': 'https://www.google.co.kr/search?hl=ko&q=%EC%98%A4%EB%8A%98%20%EB%82%A0%EC%94%A8'
            },
            'keyboard': {
                'type': 'buttons',
                'buttons': ['오늘 식단표', '내일 식단표', '다른 요일 식단표', '3학년 시간표', '학사일정', '오늘의 날씨', '홈페이지 공지사항']
...
def read_txt(meal, today, daystring):
    ...
    if meal == '3-8 내일 시간표':
        if today == 6:
            ...
            f = open('data/schedb/' + "3-8-" + str(today) + ".txt", 'r')
    if meal == '학사일정':
        f = open('data/calendardb/' + "calendar.txt", 'r')
    menu = f.read()
    f.close()
